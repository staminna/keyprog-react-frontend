...
export type CollectionItem<T extends CollectionName> = Schema[T] extends Array<infer U> 
  ? U & { id: string | number; [key: string]: unknown }
  : never;

export interface EditableItemListProps<T extends CollectionName> {
  collection: T;
  titleField?: string & keyof CollectionItem<T>;
}

export function EditableItemList<T extends CollectionName>({ 
  collection, 
  titleField = 'title' as string & keyof CollectionItem<T> 
    ...
    <div className="space-y-4">
      ...
      <ul className="space-y-2">
        {(items as CollectionItem<T>[]).map((item) => (
          <li key={item.id.toString()} className="p-4 border rounded">
