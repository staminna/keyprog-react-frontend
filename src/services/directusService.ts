...
export class DirectusService {
  ...
  static async getCurrentUser(options?: { token?: string }): Promise<User | null> {
    try {
      ...
      try {
        const response = await fetch(`${import.meta.env.VITE_DIRECTUS_URL}/users/me?fields=id,email,first_name,last_name,role.*,status,policies.*`, {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        });
        ...
        if (result && result.data) {
          ...
          if (!roleId && (!result.data.policies || result.data.policies.length === 0)) {
            ...
            if (result.data.email === envEmail) {
              // This is the environment-configured admin/editor user
              roleId = '97ef35d8-3d16-458d-8c93-78e35b7105a4'; // Editor role ID
              roleName = 'editor';
              console.log('✅ Assigned Editor role to environment user');
            } else {
              // Default to cliente role for other users
              roleId = import.meta.env.VITE_DIRECTUS_CLIENTE_ROLE_ID || '6c969db6-03d6-4240-b944-d0ba2bc56fc4';
              roleName = 'cliente';
   ...
   */
  static async activateUserAccount(userId: string): Promise<boolean> {
    try {
      const token = await sessionDirectus.getToken();
      ...
      if (!response.ok) {
        const errorData = await response.json();
        console.error('Failed to activate user account:', errorData);
        return false;
      }
